...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Sales Orders', href: '/erp/sales-orders' },
];

const page = usePage<SharedData>();
const canCreate = page.props.auth.permissions.includes('sales.create');
const canApprove = page.props.auth.permissions.includes('sales.approve');
...
function submit() {
    form.post(route('erp.sales-orders.store'), { onSuccess: () => (showCreate.value = false) });
}

const fulfillForm = useForm({});
function fulfill(order: OrderRow) {
    fulfillForm.post(route('erp.sales-orders.fulfill', order.id));
}
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-1 flex-col gap-6 p-6">
            <PageHeader title="Sales Orders" description="Orders placed by customers, fulfilled from warehouse stock.">
                <template #actions>
            ...
            <Table>
                ...
                <TableBody>
                    <TableEmpty v-if="orders.length === 0" :colspan="7">No sales orders yet.</TableEmpty>
                    <TableRow v-for="order in orders" :key="order.id">
                        ...
                        <TableCell>
                            <Badge :variant="order.status === 'fulfilled' ? 'success' : 'warning'">{{ order.status }}</Badge>
                        </TableCell>
                        <TableCell v-if="canApprove">
                            <Button v-if="order.status === 'draft'" variant="outline" size="sm" @click="fulfill(order)">Fulfill</Button>
                        </TableCell>
                    </TableRow>
                </TableBody>
            </Table>
        ...
        <Dialog :open="showCreate" @update:open="(v) => (showCreate = v)">
            <DialogContent class="max-w-2xl">
                ...
                <form class="space-y-4" @submit.prevent="submit">
                    ...
                    <div class="grid gap-2">
                        <Label for="so-date">Date</Label>
                        <Input id="so-date" v-model="form.date" type="date" />
                    ...
                    <div class="grid gap-2">
                        ...
                        <Input id="so-notes" v-model="form.notes" placeholder="Optional" />
                    </div>
