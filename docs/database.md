...
There is **no** direct foreign key between HRIS and ERP (e.g. no employee-to-warehouse or employee-to-product link). The only HRIS↔ERP relationship is indirect, through shared `users`/roles (a Warehouse Manager is a `User`, not an `Employee`, unless a demo account happens to have both).
...
- 23 of the ~40 models use the `Auditable` trait (`app/Concerns/Auditable.php`), which writes a row to `audit_logs` on every `created`/`updated`/`deleted` Eloquent event. It covers **Core and HRIS** models (`User`, `Employee`, `Department`, `Position`, `Attendance`, `LeaveRequest`, `LeaveBalance`, `LeaveType`, `OvertimeRequest`, `PayrollPeriod`, `Payslip`, `PayslipItem`, `Kpi`, `KpiCategory`, `PerformancePeriod`, `PerformanceReview`, `PerformanceReviewItem`, `Vacancy`, `Applicant`, `TrainingCategory`, `TrainingProgram`, `TrainingEnrollment`, `Account`, `JournalEntry`). **Finance's newer AR/AP tables and the entire ERP domain (`Product`, `Warehouse`, `ProductStock`, `StockMovement`, `Supplier`, `Customer`, `PurchaseOrder`, `SalesOrder`, `Invoice`, `Payable`) do not currently use the trait**, so changes there are not captured in `audit_logs`. This is a known gap, not an oversight to hide. See [Future Development](../README.md#28-future-development).
